    ...
    <div className="grid grid-cols-7 gap-1">
      {days.map((day, index) => (
        <div key={index} className="flex items-center justify-center">
          <div
            className={`relative flex flex-col items-start justify-start border border-gray-300 h-32 w-32 p-1 overflow-hidden ${day.isCurrentMonth
              ? (day.day === today ? 'bg-green-200' : 'bg-white cursor-pointer hover:bg-gray-100')
              : 'opacity-50'
              }`}
          >
            {/* Número del día */}
            ...
            <div className="mt-5 w-full h-full overflow-hidden">
              {day.events.map((event, eventIndex) => (
                <div
                  ...
                  style={{ maxWidth: '100%', maxHeight: '20px' }} // Limita la altura de cada evento
                >
                  {event.title}
                </div>
              ))}
